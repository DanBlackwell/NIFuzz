...
use libafl::{
    ...
    state::{HasCorpus, HasMetadata, StdState}, prelude::{RomuDuoJrRand, command::{CommandExecutorBuilder, StdCommandConfigurator}, CommandExecutor, CombinedExecutor},
};
...
pub fn main() {
    ...
    let all_observers = tuple_list!(time_observer, edges_observer, output_observer);
    ...
    let mut forkserver_executor = TimeoutForkserverExecutor::with_signal(
        forkserver,
    ...
    let mut cmd_executor = CommandExecutor::builder()
        .program(opt.executable)
        .timeout(Duration::from_millis(opt.timeout))
        .arg_input_file_std()
        // .build(tuple_list!(output_observer))
        .build(all_observers)
        .unwrap();
    
    let mut executor = CombinedExecutor::new(forkserver_executor, cmd_executor);
    // let () = objective;
    // let mut executor = CombinedExecutor::new::<
    //     SimpleEventManager<
    //         SimpleMonitor<impl Fn(String)>, 
    //         StdState<BytesInput, InMemoryCorpus<BytesInput>, RomuDuoJrRand, OnDiskCorpus<BytesInput>>
    //     >, 
    //     StdFuzzer<
    //         MinimizerScheduler<QueueScheduler<StdState<BytesInput, InMemoryCorpus<BytesInput>, RomuDuoJrRand, OnDiskCorpus<BytesInput>>>, LenTimeMulTestcaseScore<StdState<BytesInput, InMemoryCorpus<BytesInput>, RomuDuoJrRand, OnDiskCorpus<BytesInput>>>, MapIndexesMetadata>,
    //         CombinedFeedback<MapFeedback<DifferentIsNovel, HitcountsMapObserver<StdMapObserver<'_, u8, false>>, MaxReducer, StdState<BytesInput, InMemoryCorpus<BytesInput>, RomuDuoJrRand, OnDiskCorpus<BytesInput>>, u8>, CombinedFeedback<TimeFeedback, OutputFeedback<OutputObserver, StdState<BytesInput, InMemoryCorpus<BytesInput>, RomuDuoJrRand, OnDiskCorpus<BytesInput>>>, LogicEagerOr, StdState<BytesInput, InMemoryCorpus<BytesInput>, RomuDuoJrRand, OnDiskCorpus<BytesInput>>>, LogicEagerOr, StdState<BytesInput, InMemoryCorpus<BytesInput>, RomuDuoJrRand, OnDiskCorpus<BytesInput>>>,
    //         CombinedFeedback<CrashFeedback, MapFeedback<DifferentIsNovel, HitcountsMapObserver<StdMapObserver<'_, u8, false>>, MaxReducer, StdState<BytesInput, InMemoryCorpus<BytesInput>, RomuDuoJrRand, OnDiskCorpus<BytesInput>>, u8>, LogicFastAnd, StdState<BytesInput, InMemoryCorpus<BytesInput>, RomuDuoJrRand, OnDiskCorpus<BytesInput>>>,

    //     >>(forkserver_executor, cmd_executor);
